...
src_pts = np.float32([keypoints_1[m.queryIdx].pt for m, _ in matches]).reshape(-1, 1, 2)
dst_pts = np.float32([keypoints_2[m.trainIdx].pt for m, _ in matches]).reshape(-1, 1, 2)
F, mask = cv2.findFundamentalMat(src_pts, dst_pts, cv2.FM_RANSAC)

# Filtrer les correspondances en utilisant la matrice fondamentale
good_matches = []
...
title = 'Correspondances SIFT avec filtrage par geometrie'
